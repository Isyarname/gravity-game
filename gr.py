...
class Body:
        def __init__(self, color, pos, v):
                self.color = color
                self.pos = pos  # абсолютные координаты
                self.screen_pos = None  # экранные координаты
                self.v = v
                self.r = body_radius
                self.m = 3.14 * self.r**2
                self.F = Vector()
                self.track_points = []  # абсолютные координаты точек трека
                self.screen_track_points = []  # экранные координаты точек трека

        def apply_scale(self, pos):
                """Применяет масштабирование и смещение к позиции"""
                return ((pos * scale).round() + center).coords()

        def update_scale(self):
                """Обновляет экранные координаты при изменении масштаба"""
                self.screen_track_points = [self.apply_scale(p) for p in self.track_points]

        def draw(self):
                self.v += self.F / self.m
                self.pos += self.v
                screen_pos = self.apply_scale(self.pos)
                self.F.set([0,0])
                r = round(self.r * scale)
                r = 1 if r<1 else r
                pygame.draw.circle(surface, self.color, screen_pos, r)
                self.track_points.append(self.pos.copy())
                self.screen_track_points.append(screen_pos)
                if len(self.track_points)/2 >= 60:
                        self.track_points.pop(0)
                        self.screen_track_points.pop(0)
                i = 0
                if len(self.screen_track_points) >= 4:
                        while i+1 < len(self.screen_track_points):
                                start = self.screen_track_points[i]
                                end = self.screen_track_points[i+1]
